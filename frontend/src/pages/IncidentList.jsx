...
<<<<<<< HEAD
  ...
  return (
    <Stack spacing={2}>
      <Box component={motion.div} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        <Typography variant="h4">Libro Maestro de Incidencias</Typography>
        <Typography color="text.secondary">
          Consola operativa para filtrar, priorizar y revisar casos con criterio de continuidad del servicio.
        </Typography>
      </Box>
      ...
      <Box
        ...
        sx={{ display: 'grid', gap: 1.2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, minmax(0, 1fr))' } }}
      >
        <Chip label={`Abiertas: ${openCount}`} sx={{ justifyContent: 'flex-start', fontWeight: 700, bgcolor: '#FFF5E8', color: '#9A3412' }} />
        <Chip label={`Escaladas: ${escalatedCount}`} sx={{ justifyContent: 'flex-start', fontWeight: 700, bgcolor: '#FFF1F2', color: '#9F1239' }} />
        <Chip label={`Con solucion: ${withSolutionCount}`} sx={{ justifyContent: 'flex-start', fontWeight: 700, bgcolor: '#ECFDF3', color: '#166534' }} />
      </Box>
