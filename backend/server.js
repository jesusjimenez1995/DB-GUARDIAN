...
async function fetchResolvedIncidentPool(connection) {
  if (useMocks) {
    return mockIncidents.filter((incident) =>
      ['SOLUCIONADA', 'CERRADA'].includes(incident.ESTADO) && String(incident.SOLUCION || '').trim()
    );
  }

  const result = await connection.execute(
    `SELECT ID, APP_NAME, ERROR_CODE, TITULO, DESCRIPCION, ESTADO, PRIORIDAD, ASIGNADO_A,
            SOLUCION, FECHA_ALTA, FECHA_RESOLUCION
     FROM INCIDENCIAS
     WHERE ESTADO IN ('SOLUCIONADA', 'CERRADA')
...
app.get('/api/incidents/:id/recommendations', async (request, reply) => {
  const id = Number(request.params.id);
  if (!id || Number.isNaN(id)) {
    return reply.code(400).send({ error: 'Invalid incident id' });
  }
  ...
  if (useMocks) {
    const source = findIncidentById(id);
    if (!source) return reply.code(404).send({ error: 'Incident not found' });
    return { data: rankResolvedIncidents(source, mockIncidents, { excludeId: id }) };
  }
  ...
  try {
    ...
    const currentResult = await connection.execute(
      `SELECT ID, APP_NAME, ERROR_CODE, TITULO, DESCRIPCION
       FROM INCIDENCIAS
       WHERE ID = :id`,
      { id }
    );
    ...
    const source = currentResult.rows[0];
    const pool = await fetchResolvedIncidentPool(connection);
    return { data: rankResolvedIncidents(source, pool, { excludeId: id }) };
  } catch (error) {
    app.log.error(error, 'Failed to fetch recommendations');
    return reply.code(500).send({ error: 'Error fetching recommendations' });
  } finally {
    if (connection) await connection.close();
  }
});
...
app.post('/api/recommendations', async (request, reply) => {
  ...
  if (useMocks) {
    return { data: rankResolvedIncidents(source, mockIncidents, { excludeId: source.excludeId }) };
  }
  ...
  try {
    connection = await getConnection();
    const pool = await fetchResolvedIncidentPool(connection);
    return { data: rankResolvedIncidents(source, pool, { excludeId: source.excludeId }) };
  } catch (error) {
    app.log.error(error, 'Failed to build recommendation list');
    return reply.code(500).send({ error: 'Error fetching recommendations' });
  } finally {
    if (connection) await connection.close();
  }
